...
class DiffusionChainCov(nn.Module):
    ...
    def forward(
        ...
        if self.X_prior is None:
            C_prior = torch.ones([32, 8000]).to(C.device)
            self.X_prior = self.base_gaussian.sample(C_prior)
        X_prior = copy.deepcopy(self.X_prior[:X.shape[0], :X.shape[1]])
        X_prior[C==0.] = torch.zeros(4,3).to(X.device)
...
class ReconstructionLosses(nn.Module):
    ...
    def _batch_average(self, loss, C):
        ...
        mask = (torch.isnan(loss)==False)
        return (weights[mask] * loss[mask]).sum() / (weights[mask].sum() + self._loss_eps)
